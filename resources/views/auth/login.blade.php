...
<head>
    ...
    <title>Espace Connexion | AXA</title>
...
<body class="min-h-screen bg-slate-100 font-sans antialiased flex flex-col justify-between">
    ...
    <div class="relative w-full bg-navy text-white pt-12 pb-32 overflow-hidden custom-wave">
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-4 flex flex-col items-center text-center">
            ...
            <h1 class="text-3xl md:text-4xl font-bold tracking-tight">Espace Connexion</h1>
            <p class="text-slate-200 mt-2 text-sm md:text-base font-light">Accès sécurisé</p>
        </div>
    </div>
        ...
        <div class="w-full max-w-xl bg-white rounded-2xl shadow-xl p-6 md:p-8 border border-slate-100">
            ...
            <div class="flex items-center gap-3 mb-6">
                ...
                <div class="flex-grow flex items-center gap-4">
                    <h2 class="text-sm font-bold text-navy uppercase tracking-wider whitespace-nowrap">Authentification</h2>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="password"
                               ...
                               class="w-full pl-10 pr-24 py-2.5 bg-white border border-slate-300 rounded-lg text-sm transition-all focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy placeholder-slate-400 @error('password') border-red-500 ring-2 ring-red-100 @enderror">
                        
                        <button type="button"
                                onclick="togglePassword()"
                                class="absolute inset-y-0 right-0 pr-3 flex items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-navy transition-colors focus:outline-none">
                            <i id="toggle-icon" data-lucide="eye" class="w-4 h-4"></i>
                ...
                <div class="flex items-center justify-between text-sm pt-1">
                    <label class="flex items-center gap-2 cursor-pointer text-slate-600 select-none">
                        <input type="checkbox" name="remember" class="w-4 h-4 rounded border-slate-300 text-navy focus:ring-navy/20 accent-navy cursor-pointer">
                        <span class="text-sm font-medium">Se souvenir de moi</span>
                    </label>

                    <a href="{{ route('password.request') }}" class="text-sm font-medium text-slate-600 hover:text-navy hover:underline transition-colors">
                        Mot de passe oublié ?
                    </a>
                </div>
    ...
    <script>
        ...
        function togglePassword() {
            ...
            if (input.type === 'password') {
                ...
                textBtn.innerText = 'Masquer';
            } else {
                input.type = 'password';
                iconBtn.setAttribute('data-lucide', 'eye');
                textBtn.innerText = 'Afficher';
            }
            lucide.createIcons();
        }
    </script>
