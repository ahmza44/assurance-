    ...
    <main class="flex-grow-1 p-4">
        <div class="container-fluid px-0">
            <div class="mb-4 pb-2 page-header">
                <h2 class="fw-semibold">Modifier un établissement</h2>
            </div>
            ...
            <div class="card card-form shadow-sm">
                <div class="card-body p-4">
                    <form action="{{ route('admin.establishments.update', $establishment->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="university_id" class="form-label">Université Parente</label>
                            <select class="form-select @error('university_id') is-invalid @enderror" id="university_id" name="university_id" required>
                                ...
                                @foreach ($universities as $university)
                                    <option value="{{ $university->id }}" {{ old('university_id', $establishment->university_id) == $university->id ? 'selected' : '' }}>{{ $university->name }}</option>
                                @endforeach
                            </select>
                            @error('university_id')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
